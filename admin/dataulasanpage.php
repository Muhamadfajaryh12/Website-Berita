...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <div class="container-fluid">
    ...
    <form class="d-flex" method="post">
      <input class="form-control me-2" type="text" name="keyword" placeholder="Cari Nama ID" aria-label="Search">
...
$keyword=$_POST['keyword'];
    if ($keyword !=''){
        $query = mysqli_query($koneksi, "SELECT * FROM ulasan INNER JOIN user ON ulasan.id_user=user.id WHERE id_user LIKE '%$keyword%'");
        
    }else{
        $query = mysqli_query($koneksi, "SELECT * FROM ulasan INNER JOIN user ON ulasan.id_user=user.id ORDER BY id_ulasan ASC LIMIT $start_from, $limit ");
    }
    while ($data=mysqli_fetch_array($query)) {
?>
                                    <tr>
                                        <th scope="row"><?= $no++ ?></th>
                                        <td style="width:150px; font-size:15px;"><b><?= $data['username'] ?><b></td>
                                        <td><?= $data['email'] ?></td>
                                        <td><?= $data['judul_ulasan']?></td>
                                        <td>
                                            <div class="btn-group" style="margin-top:2px;">
                                                <a href="viewulasanpage.php?id_ulasan=<?= $data['id_ulasan'] ?>" class="btn btn-dark" style="width:70px; font-size:12px; margin:2px;">Baca</a>
                                                <a href="deleteulasanpage.php?id_ulasan=<?= $data['id_ulasan'] ?>" class="btn btn-secondary delete-link" style="width:70px; font-size:12px; margin:2px;">Delete</a>
                                            </div>
                                        </td>
                                    </tr>
                                    <?php
                            ...
                            <?php
                            $result_db = mysqli_query($koneksi,"SELECT COUNT(*) as jumlah FROM ulasan"); 
